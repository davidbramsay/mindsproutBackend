...
class audioInterface(object):
    ...
    def processNext(self, filepath, current="silence"):
        return filepath, current

    #analyze silence of given audio file, update mongodb, and return results
    def processSilence(self, filepath):
        returnVal = silence.process(filepath)
        return returnVal

    #analyze silence of given audio file, update mongodb, and return results
    def processVolume(self, filepath):
        returnVal = volume.process(filepath)
        return returnVal

    #analyze silence of given audio file, update mongodb, and return results
    def processIntonation(self, filepath):
        returnVal = intonation.process(filepath)
        return returnVal

    #analyze silence of given audio file, update mongodb, and return results
    def processWordsSpoken(self, filepath):
        returnVal = wordsspoken.process(filepath)
        return returnVal

    #check ZMQ link is function properly and we are getting round-trip comm
    def verifyLink(self, returnString):
        return "python ZMQ roundtrip test... {0}".format(returnString)
